...
/**
 * Handles database operations for the Pet.
 */
class PetRepository {
  /**
   * Creates the Pet.
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Pet().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  /**
   * Counts the number of Pets based on the filter.
   *
   * @param {Object} filter
   */
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Pet().collectionName);
  ...
  /**
   * Finds the Pet and its relations.
   *
   * @param {string} id
   */
  async findById(id) {
  ...
  /**
   * Finds the Pets based on the query.
  ...
  /**
   * Lists the Pets to populate the autocomplete.
